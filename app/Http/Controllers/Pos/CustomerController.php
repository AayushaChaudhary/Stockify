...
class CustomerController extends Controller
{
    public function CustomerAll(){
        ...
        $customers = Customer::leftJoin('payments', 'customers.id', '=', 'payments.customer_id')
            ->select('customers.*', DB::raw('SUM(payments.total_amount) as total_payment'))
            ->groupBy('customers.id')
            ->orderByDesc('total_payment')
            ->get();

        $customers = $customers->map(function ($customer) {
            $customer->total_payment = $customer->total_payment ?? 0;
            return $customer;
        });
